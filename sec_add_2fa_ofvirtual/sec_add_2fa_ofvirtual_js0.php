...
<form name="F5" method="post" 
  ...
  <input type="hidden" name="nmgp_parms" value="<?php if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_add_2fa_ofvirtual']['parms'])) {echo $this->form_encode_input($_SESSION['sc_session'][$this->Ini->sc_page]['sec_add_2fa_ofvirtual']['parms']);} ?>"/>
  <input type="hidden" name="script_case_init" value="<?php echo $this->form_encode_input($this->Ini->sc_page); ?>"/> 
</form> 
...
<form name="FCAP" action="" method="post" target="_blank"> 
  <input type="hidden" name="SC_lig_apl_orig" value="sec_add_2fa_ofvirtual"/>
  <input type="hidden" name="nmgp_parms" value=""> 
...
?> 
function sc_btn_send_sms()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_send_sms(); }, 500);
      return;
    }
    sc_btn_send_sms_ok();
}
function sc_btn_send_sms_cancel()
{
}
function sc_btn_send_sms_ok()
{
    do_ajax_sec_add_2fa_ofvirtual_event_scajaxbutton_send_sms_onclick();
}
function sc_btn_send_email()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_send_email(); }, 500);
      return;
    }
    sc_btn_send_email_ok();
}
function sc_btn_send_email_cancel()
{
}
function sc_btn_send_email_ok()
{
    do_ajax_sec_add_2fa_ofvirtual_event_scajaxbutton_send_email_onclick();
}
function sc_btn_reset()
{
    if (scEventControl_active("")) {
      setTimeout(function() { sc_btn_reset(); }, 500);
      return;
    }
    sc_btn_reset_ok();
}
function sc_btn_reset_cancel()
{
}
function sc_btn_reset_ok()
{
...
<?php
  if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['sec_add_2fa_ofvirtual']['where_detal']))
  {
?>  
...
function sc_inline_form(seqRow, keyParams, width, height)
{
  ...
  nm_gp_submit('<?php echo $this->Ini->link_sec_add_2fa_ofvirtual_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinN*scoutNM_btn_update*scinS*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinigual*scoutsc_seq_row*scin' + seqRow + '*scout' + callParams, '', 'modal', height, width);
}

function sc_inline_form_add(width, height)
{
  nm_gp_submit('<?php echo $this->Ini->link_sec_add_2fa_ofvirtual_inline ?>', '<?php echo $this->nm_location ?>', 'NM_btn_insert*scinY*scoutNM_btn_update*scinN*scoutNM_btn_delete*scinN*scoutNM_btn_navega*scinN*scoutNMSC_modal*scinok*scoutsc_redir_atualiz*scinok*scoutsc_inline_call*scinY*scoutnmgp_opcao*scinnovo*scout', '', 'modal', height, width);
}
...
{
  if (scFormHasChanged) {
    scJs_confirm('<?php echo html_entity_decode($this->Ini->Nm_lang['lang_reload_confirm']) ?>', function() { document.F4.target = "_self"; document.F4.action = "sec_add_2fa_ofvirtual_fim.php"; document.F4.submit(); }, function() {});
  } else {
    document.F4.target = "_self";
    document.F4.action = "sec_add_2fa_ofvirtual_fim.php";
    document.F4.submit();
  }
}
