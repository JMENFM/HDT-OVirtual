...
$old_error_handler = set_error_handler("ofvirtual_scriptcase_error_handler");

function ofvirtual_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe="", $err_usr=false)
{
    ...
    if (isset($_SESSION['scriptcase']['ofvirtual']['contr_erro']) && $_SESSION['scriptcase']['ofvirtual']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_ofvirtual->contr_ofvirtual->Apl_com_erro = true;
            if ($inicial_ofvirtual->contr_ofvirtual->NM_ajax_flag)
            {
                $inicial_ofvirtual->contr_ofvirtual->NM_ajax_info['errList']['geral_ofvirtual'][] = $msg;
                return;
            }
            ofvirtual_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function ofvirtual_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['ofvirtual'])  && '' != $_SESSION['scriptcase']['error_icon']['ofvirtual'])  ? $_SESSION['scriptcase']['error_icon']['ofvirtual']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['ofvirtual']) && '' != $_SESSION['scriptcase']['error_close']['ofvirtual']) ? $_SESSION['scriptcase']['error_close']['ofvirtual'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['ofvirtual']) && '' != $_SESSION['scriptcase']['error_span_title']['ofvirtual']) ? $_SESSION['scriptcase']['error_span_title']['ofvirtual'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['ofvirtual']) && '' != $_SESSION['scriptcase']['error_icon_title']['ofvirtual']) ? $_SESSION['scriptcase']['error_icon_title']['ofvirtual'] : '';
?>
...
<?php
    ...
    $_SESSION['scriptcase']['ofvirtual']['error_buffer'] .= ob_get_flush();
}
